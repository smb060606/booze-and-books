-- Fix notification trigger to use cancelled_by instead of auth.uid()
-- This addresses the issue where auth.uid() can be NULL in service contexts

-- Update the notification function to use cancelled_by instead of auth.uid()
...
-- Add comments
COMMENT ON FUNCTION create_swap_request_notification() IS 'Enhanced notification function that uses cancelled_by column to avoid auth.uid() issues in service contexts';